        ...
        selectedTicket?.let {
            if (it.ownerUsername == null) {
                ...
                lifecycleScope.launch(Dispatchers.IO) {
                    ...
                    withContext(Dispatchers.Main) {
                        Toast.makeText(this@MovieDetailsActivity, "Ticket bought", Toast.LENGTH_SHORT).show()
                        loadTickets()  // Обновим список билетов
                    }
                }
            } else {
                Toast.makeText(this, "This ticket is already taken", Toast.LENGTH_SHORT).show()
            }
        } ?: run {
            Toast.makeText(this, "Select a ticket first", Toast.LENGTH_SHORT).show()
        }
    }
}
