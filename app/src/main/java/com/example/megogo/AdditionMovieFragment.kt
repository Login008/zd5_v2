...
class AdditionMovieFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        movieViewModel.movieAdded.observe(viewLifecycleOwner, Observer { success ->
            if (success) {
                ...
                Toast.makeText(requireContext(), "There's a error during addition of the movie", Toast.LENGTH_SHORT).show()
            }
        })
    ...
    private fun searchMovies(title: String, year: String) {
        ...
        val stringRequest = StringRequest(
            ...
            {
                Toast.makeText(requireContext(), "There's some error, we're soon gonna fix it", Toast.LENGTH_SHORT).show()
            }
        )
    ...
    private fun addMovieToDatabase(movie: Movie?) {
        if (movie == null) {
            Toast.makeText(requireContext(), "Please search and select a movie first", Toast.LENGTH_SHORT).show()
            return
        }
            ...
            try {
                ...
                if (existingMovie == null) {
                    ...
                    withContext(Dispatchers.Main) {
                        Toast.makeText(requireContext(), "The movie is already in the rental", Toast.LENGTH_SHORT).show()
                    }
                }
            } catch (e: Exception) {
                withContext(Dispatchers.Main) {
